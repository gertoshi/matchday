...
type Equipo = {
    ...
    inscripciones?: {
        ...
        evento?: { nombre_evento: string } | null;
    }[];
};
...
export default function DetalleEquipo({ equipo }: Props) {
    function suspender() {
        router.post(
            `/admin/equipos/${equipo.id}/suspender`,
            {},
            { preserveScroll: true },
        );
    }
    ...
    function eliminar() {
        if (
            window.confirm(`¿Marcar como eliminado a ${equipo.nombre_equipo}?`)
        ) {
            router.delete(`/admin/equipos/${equipo.id}`, {
                preserveScroll: true,
            });
        }
    }
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-gray-500">
                                    {equipo.user?.name ?? 'Sin responsable'}
                                </p>
                            </div>
                        </div>
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                    <section className="app-card">
                        ...
                        <div className="mt-6 space-y-3">
                            <InfoRow
                                label="Nombre"
                                value={equipo.user?.name ?? 'Sin usuario'}
                            />
                            <InfoRow
                                label="Email"
                                value={equipo.user?.email ?? 'Sin email'}
                            />
                            <InfoRow
                                label="Teléfono"
                                value={equipo.user?.phone ?? 'Sin teléfono'}
                            />
                        </div>
                    </section>
                    ...
                    <section className="app-card">
                        ...
                        <div className="mt-6 space-y-3">
                            <InfoRow
                                label="Plantilla declarada"
                                value={String(equipo.plantilla)}
                            />
                            <InfoRow
                                label="Jugadores cargados"
                                value={String(equipo.jugadores?.length ?? 0)}
                            />
                            <InfoRow
                                label="Inscripciones"
                                value={String(
                ...
                <section className="app-card">
                    ...
                    <div className="mt-6 overflow-x-auto">
                        ...
                        {(equipo.jugadores ?? []).length === 0 ? (
                            <Empty text="No hay jugadores cargados." />
                        ) : null}
                    </div>
                </section>
            </div>
